 ...
 * @author Lars Doucet
 */
class ArtPreviewState extends FlxState
{
	...
	override public function create():Void
	{
		...
		FlxG.cameras.bgColor = PlayState.backgroundColor;

		_img = new FlxSprite(0, 0);
		_img.makeGraphic(FlxG.width, FlxG.height, PlayState.backgroundColor);
		FlxSpriteUtil.fill(_img, FlxColor.TRANSPARENT);
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (FlxG.mouse.pressed) {
			_img.x += FlxG.mouse.deltaScreenX;
			_img.y += FlxG.mouse.deltaScreenY;
		}
	...
	function _showImage(Data:BitmapData):Void
	{
		...
		_img.makeGraphic(Std.int(_displayWidth), Std.int(_displayHeight), FlxColor.TRANSPARENT);
